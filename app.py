...
# --- SHARED UTILITY: COLUMN MERGING & CLEANING ---
def consolidate_columns(df):
    """
    Standardizes headers and merges redundant columns. 
    Fixes the ValueError by ensuring unique column names after merging.
    """
    # 1. Clean headers to remove hidden spaces/double spaces
    df.columns = df.columns.str.strip().str.replace(r'\s+', ' ', regex=True)
    
    # 2. Handle physical duplicates in the CSV (e.g., two columns named 'Emp ID')
    ...
    for target_col, variations in mapping.items():
        # Identify which variants exist in the file
        existing_cols = [col for col in variations if col in df.columns]
        if existing_cols:
            # Create the consolidated series by taking the first non-null value
...
# --- CUSTOM LOGIC: DUTY DETAIL RENAMING ---
def refine_duty_detail(row):
    ...
    # 2. Tiered Package Logic for all other cities
    try:
...
def process_bdc_automation(file):
    ...
    def apply_billing(row):
        ...
        if is_special:
            ...
            # Use 'Sales Extra Hour Rate' for km charges as requested
...
with tab3:
    ...
    if 'df_auto' in st.session_state:
        ...
        st.warning("Please upload a file in the 'BDC Automation' tab first.")def refine_duty_detail(row):
    """
    Applies special city logic and tiered package renaming.
    """
    city = str(row.get('Pickup City', ''))
    duty_type = str(row.get('Duty Type', ''))
    package = str(row.get('Duty Package', ''))
...
with tab2:
    st.header("BDC Data Analysis")
    file2 = st.file_uploader("Upload Raw Data (CSV)", type=["csv"], key="bdc")
    if file2:
        df_full, df_auto = process_bdc_automation(file2)
        st.session_state['df_auto'] = df_auto
        st.success("Calculations Complete!")
...
with tab3:
    st.header("AutoBDC Final Format")
    if 'df_auto' in st.session_state:
        st.dataframe(st.session_state['df_auto'])
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            st.session_state['df_auto'].to_excel(writer, sheet_name='AutoBDC', index=False)
        st.download_button(label="⬇️ Download AutoBDC Excel", data=output.getvalue(), file_name="AutoBDC_Final.xlsx")
    else:
